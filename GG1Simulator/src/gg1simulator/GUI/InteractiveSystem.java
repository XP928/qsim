/*
 * InteractiveSystem.java
 */
package gg1simulator.GUI;

import gg1simulator.Distribution;
import gg1simulator.InteractiveSystemCalculator;
import gg1simulator.InteractiveSystemSimuator;
...
public class InteractiveSystem extends Simulator {
    ...
    @Override
    public void init() {
        super.setImageName("InteractiveSystem.jpg");
        super.setNumOfServers(1);
        super.setModelType(3);
        super.init();
        ((JTextField) getColtrolsMap().get(getNumOfTerminals())).setText("3");
        ((JTextField) getColtrolsMap().get(getNumOfServers())).setText("1");
    }
    ...
    public JPanel createSimButtons() {
        ...
        btnSim.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                n = Integer.parseInt(((JTextField) getColtrolsMap().get(getNumOfTerminals())).getText());
                ...
                InteractiveSystemSimuator sim = new InteractiveSystemSimuator(thinkTimeDistribution, serviceDistribution, n, m, bars, interval);
                sim.run(steps);

                mainResultPanel.removeAll();
                mainResultPanel.add(createTitle());

                InteractiveSystemCalculator c = new InteractiveSystemCalculator(thinkTimeDistribution.getMean(), serviceDistribution.getMean(), n, m);
                c.Calculate();
                ...
                errorStr = "";
                mainResultPanel.add(createRow("Interarrival time", avg, sdv, "", errorStr));
                ...
                avg = sim.getAvgCycleTime();
                sdv = sim.getSDCycleTime();
                realAvg = c.getCycleTime();
                realValue = Utility.formatOutput(realAvg);
                error = 100 * (avg - realAvg) / realAvg;
                if (temp == 1) {
                    error *= -1;
                ...
                errorStr = Utility.formatOutput(error);
                mainResultPanel.add(createRow("Cycle time", avg, sdv, realValue, errorStr));
                
                avg = n / avg;
                mainResultPanel.add(createRow("Thoughput", avg, "", ""));
                ...
                if (flag) {
                    myChartPanel.add(createChartPanel(preCycleTime, departures, preInterval, "Cycle time (Previous Exp)", 0));
                }
                myChartPanel.add(createChartPanel(sim.getCycleTimeList(), sim.getDepartures(), interval, "Cycle time", 1));
                ...
                System.arraycopy(sim.getCycleTimeList(), 0, preCycleTime, 0, bars);
    ...
    public boolean checkErrors() {
        ...
        try {
            ...
            errorName = "the number of terminals ";
            int m = Integer.parseInt(((JTextField) getColtrolsMap().get(getNumOfTerminals())).getText());
            if (m <= 0) {
                errorName += "must be a postive integer";
                JOptionPane.showMessageDialog(null, errorName);
                return true;
            ...
            int n = Integer.parseInt(((JTextField) getColtrolsMap().get(getNumOfServers())).getText());
            if (n <= 0) {
                errorName += "must be a postive integer";
                JOptionPane.showMessageDialog(null, errorName);
                return true;
            ...
            if (m - n < 0) {
                JOptionPane.showMessageDialog(null, "the number of terminals must greater than the number of servers");
                return true;
            }
        } catch (Exception ex) {
